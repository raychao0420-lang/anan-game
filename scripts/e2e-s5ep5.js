// E2E：S5 EP5《大草原的遷徙地圖》全流程（無兇手集：真相=改道的基庫尤河）
// 預埋 EP1~4 已破存檔解鎖 EP5 → 8 現場（含分支）→ 先錯指爺爺→指認河 → 驗證肯亞章+金幣
// 執行：先 `npm run dev`，複製本檔到裝有 playwright-core 的目錄跑 `node e2e-s5ep5.js`
...
const ANSWERS = [1000, 3500, 4, null, 50, 4700, 270, 1000]
const BRANCH = { pick: 0, answer: 6 } // 左：水源 2+4=6 公里（右：斑馬路標 6000m=6 公里 同答案）
...
;(async () => {
  ...
  // 預埋：EP1~4 已破＋東京/印度/埃及/義大利章已收（zustand persist 淺合併，其餘用預設值）
  await page.addInitScript(() => {
    localStorage.setItem('anan-game-v2', JSON.stringify({
      state: { seriesSolved: { s5ep1: true, s5ep2: true, s5ep3: true, s5ep4: true }, seriesStamps: ['tokyo', 'india', 'egypt', 'italy'] }, version: 0,
    }))
  })
  ...
  const ep5 = page.locator('.srs-ep-card', { hasText: '第 5 集' })
  if ((await ep5.innerText()).includes('🔒')) fail('EP5 應解鎖卻是鎖住的')
  await ep5.click()

  await page.waitForSelector('text=開始查案')
  const introText = await page.locator('.dtv-panel', { hasText: '開始查案' }).innerText()
  if (!introText.includes('委託電報第 5 號')) fail('EP5 開場沒有巡巡電報')
  ...
  // 指認（無兇手集：找出真相）：證據板 8 條 → 故意指錯（爺爺）→ 指認改道的基庫尤河
  ...
  await clickText(page, '.dtv-suspect', '爺爺')
  await page.waitForSelector('.dtv-hint')
  await clickText(page, '.dtv-suspect', '基庫尤')

  await page.waitForSelector('.srs-got-shard')
  const got = await page.locator('.srs-got-shard').innerText()
  if (!got.includes('肯亞章')) fail('沒拿到肯亞章：' + got)

  const st = await page.evaluate(() => JSON.parse(localStorage.getItem('anan-game-v2')).state)
  if (!st.seriesStamps?.includes('kenya')) fail('seriesStamps 沒收到 kenya：' + JSON.stringify(st.seriesStamps))
  if (!st.seriesSolved?.s5ep5) fail('seriesSolved.s5ep5 沒標記')
  if (st.coins !== coinsBefore + 660) fail(`金幣應 +660（${coinsBefore}→${coinsBefore + 660}），實際 ${st.coins}`)

  // 回列表：護照 5/12＋感謝卡線索含 EP5
  ...
  if (!wall.includes('5/12')) fail('護照應顯示 5/12：' + wall)
  const clue = await page.locator('.srs-wall').nth(1).innerText().catch(() => '')
  if (!clue.includes('EP5')) fail('感謝卡線索牆沒有 EP5')

  console.log(process.exitCode ? 'E2E 有失敗項目' : '✅ E2E 全部通過：S5 EP5 流程＋肯亞章＋金幣＋感謝卡線索都正常')
  await browser.close()
