...
const ROUNDS = [
  { no: 1, timeLimit: 20, label: '熱身', color: '#4CAF50', bg: '#F1F8E9' },
  { no: 2, timeLimit: 12, label: '加速', color: '#FF9800', bg: '#FFF3E0' },
  { no: 3, timeLimit: 7,  label: '極速', color: '#F44336', bg: '#FFEBEE' },
]
...
export default function MakeTwentyScreen({ onBack }) {
  const { activePet, pets, petEquipment, makeTwentyCleared, clearMakeTwenty, updatePetMood, addCoins } = useGameStore()
  ...
  }, [phase, feedback, input, currentQ, qIdx, roundIdx, round, makeTwentyCleared, clearMakeTwenty, activePet, updatePetMood])
  ...
  if (phase === 'intro') return (
    <div className="mt-screen" style={{ background: '#E8F5E9' }}>
      ...
      >
        <div className="mt-main-title">🔢 湊20特訓</div>
        ...
        <p className="mt-desc">
          10題全對才能過關！<br />
          連過三關就能得到傳說道具<br />
          <b style={{ color: '#1565C0', fontSize: '1.1rem' }}>✌️ 雙指勝利</b>！
        </p>
        ...
        {makeTwentyCleared && (
          <div className="mt-cleared-badge">✅ 已獲得雙指勝利！可再次挑戰</div>
        )}
    ...
    <div className="mt-screen" style={{ background: 'linear-gradient(160deg, #E3F2FD, #BBDEFB)' }}>
      ...
      >
        ...
        >
          ...
          <div className="mt-reward-name">雙指勝利</div>
          <div className="mt-reward-sub">進位心算高手！</div>
          {wonFirst && (
