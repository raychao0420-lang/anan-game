...
// ── 兩位加減深化（71-80）─────────────────────────────────────
generators[71] = () => {                                           // 必進位，各種大小
  const aO = rand(3,9); const bO = rand(10-aO, 9)
  const aT = rand(1,7); const bT = rand(1, Math.min(8, 9-aT))
  return makeAdd(aT*10+aO, bT*10+bO)
}
generators[72] = () => {                                           // 必退位，各種大小
  const aO = rand(0,7); const bO = rand(aO+1, 9)
  const aT = rand(2,9); const bT = rand(1, aT-1)
  return makeSub(aT*10+aO, bT*10+bO)
}
generators[73] = () => {                                           // 大數必進位（被加數 30+）
  const aO = rand(5,9); const bO = rand(10-aO, 9)
  const aT = rand(3,8); const bT = rand(1, 9-aT)
  return makeAdd(aT*10+aO, bT*10+bO)
}
generators[74] = () => {                                           // 大數必退位（被減數 30+）
  const aO = rand(0,5); const bO = rand(aO+2, 9)
  const aT = rand(3,9); const bT = rand(1, aT-1)
  return makeSub(aT*10+aO, bT*10+bO)
}
generators[75] = () => {                                           // 兩數均 40+，加法
  const a = rand(41,79); const b = rand(14, 99-a)
  return makeAdd(a, b)
}
generators[76] = () => {                                           // 被減數 60+，減數 20+
  const a = rand(62,99); const b = rand(21, a-21)
  return makeSub(a, b)
}
generators[77] = () => {                                           // 加法特訓（33-69 + 16+）
  const a = rand(33,69); const b = rand(16, 99-a)
  return makeAdd(a, b)
}
generators[78] = () => {                                           // 減法特訓（44-99 - 16+）
  const a = rand(44,99); const b = rand(16, a-16)
  return makeSub(a, b)
}
generators[79] = () => (rand(0,1) ? generators[77]() : generators[78]()) // 加減混合特訓
generators[80] = () => {                                           // 兩位數加減大挑戰
  const ops = [generators[71], generators[72], generators[73], generators[74], generators[75], generators[76]]
  return ops[rand(0, 5)]()
}

// ── 乘除深化（86-95）─────────────────────────────────────────
generators[86] = () => makeMul(rand(11,19), rand(5,9))             // 十幾 × 大數
generators[87] = () => makeMul(rand(20,29), rand(2,9))             // 二十幾 × 一位
generators[88] = () => makeMul(rand(30,49), rand(2,9))             // 三四十幾 × 一位
generators[89] = () => makeMul(rand(50,99), rand(2,9))             // 大數 × 一位
generators[90] = () => {                                           // 乘法大挑戰
  const ops = [generators[86], generators[87], generators[88], generators[89]]
  return ops[rand(0, 3)]()
}
generators[91] = () => {                                           // 兩位 ÷ 一位，商一位
  const b = rand(2,9); const a = rand(2, Math.min(9, Math.floor(99/b)))
  return makeDiv(a, b)
}
generators[92] = () => {                                           // 兩位 ÷ 一位，商兩位
  const b = rand(2,5); const maxA = Math.floor(99/b)
  return makeDiv(rand(Math.max(10, Math.ceil(10/b)), maxA), b)
}
generators[93] = () => (rand(0,1) ? makeMul(rand(11,69), rand(2,9)) : generators[91]()) // 乘除混合速算
generators[94] = () => {                                           // 乘除大挑戰
  const r = rand(0,3)
  if (r===0) return generators[88]()
  if (r===1) return generators[89]()
  if (r===2) return generators[91]()
  return generators[92]()
}
generators[95] = () => {                                           // 乘除終極
  const r = rand(0,4)
  if (r===0) return generators[86]()
  if (r===1) return generators[88]()
  if (r===2) return generators[89]()
  if (r===3) return generators[91]()
  return generators[92]()
}

// ── 兩位數乘法：兩位數 × 兩位數（96-100）·翰林四上單元，每題給 90 秒 ─────
generators[96]  = () => makeMul(rand(11, 19), rand(11, 19))        // 十幾 × 十幾
generators[97]  = () => makeMul(rand(21, 49), rand(11, 19))        // 兩位 × 十幾
generators[98]  = () => makeMul(rand(11, 59), rand(21, 29))        // 兩位 × 廿幾
generators[99]  = () => makeMul(rand(21, 69), rand(21, 69))        // 兩位 × 兩位（中範圍）
generators[100] = () => {                                          // 兩位乘法大挑戰（含全範圍）
  const ops = [generators[96], generators[97], generators[98], generators[99],
               () => makeMul(rand(21, 99), rand(21, 99))]
  return ops[rand(0, 4)]()
}
...
function makeWarmup(operator) {
  switch (operator) {
    ...
    default:  return makeAdd(rand(1, 5), rand(1, 5))          // 不進位小加法
  }
}
...
export function generateStageQuestions(stageId) {
  ...
  while (questions.length < WARMUP_COUNT && warmAttempts < 50) {
    ...
    if (!seen.has(q.display) && q.answer > 0) {
      seen.add(q.display)
      questions.push(q)
    }
    warmAttempts++
...
STAGE_NAMES[73] = '大數進位'
STAGE_NAMES[74] = '大數退位'
STAGE_NAMES[75] = '加法馬拉松'
STAGE_NAMES[76] = '減法馬拉松'
STAGE_NAMES[77] = '加法特訓'
STAGE_NAMES[78] = '減法特訓'
STAGE_NAMES[79] = '加減混合特訓'
STAGE_NAMES[80] = '🏅 兩位數大挑戰'

// 乘除深化
STAGE_NAMES[86] = '十幾乘大數'
STAGE_NAMES[87] = '二十幾乘法'
STAGE_NAMES[88] = '三四十乘法'
STAGE_NAMES[89] = '大數乘法'
STAGE_NAMES[90] = '乘法大挑戰'
STAGE_NAMES[91] = '整除衝刺'
STAGE_NAMES[92] = '兩位除一位'
STAGE_NAMES[93] = '乘除速算'
STAGE_NAMES[94] = '乘除大挑戰'
STAGE_NAMES[95] = '🏆 乘除終極'

// 兩位數乘法（× 兩位數）
STAGE_NAMES[96]  = '十幾乘十幾'
STAGE_NAMES[97]  = '兩位乘十幾'
